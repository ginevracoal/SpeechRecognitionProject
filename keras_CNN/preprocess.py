...
def save_data_to_array(path=DATA_PATH, max_len=11):
    ...
    for label in labels:
        ...
        # wavfiles = []
        # for label in labels:
        #     for wavfile in os.listdir(DATA_PATH + '/' + label):
        #         if wavfile.endswith(".wav"):
        #             wavfiles.append(DATA_PATH + label + '/' + wavfile)
        ...
        np.save(out_dir + label + '.npy', mfcc_vectors)
...
def get_train_test(split_ratio=0.6, random_state=42):
    ...
    X = np.load(out_dir + labels[0] + '.npy')
    # X = np.load(os.path.join('keras_CNN','arrays', label[0] + '.npy'))
    y = np.zeros(X.shape[0])

    # Append all of the dataset into one single array, same goes for y
    for i, label in enumerate(labels[1:]):
        x = np.load(out_dir + label + '.npy')
        X = np.vstack((X, x))
